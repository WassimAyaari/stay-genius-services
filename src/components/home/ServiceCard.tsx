...
const ServiceCard = ({ 
  ...
  return (
    <Link to={actionLink} className="block h-full">
      <Card className={`h-full overflow-hidden transition-all duration-300 hover:shadow-lg ${highlighted ? 'border-2 border-primary rounded-2xl' : 'rounded-2xl'} hover:scale-[1.02] bg-card border-border`}>
        <div className="p-3 sm:p-4 flex flex-col h-full min-h-[140px] sm:min-h-[160px] bg-card">
          ...
          <div className="flex-1 flex flex-col">
            <h3 className="text-base sm:text-xl font-bold text-card-foreground mb-1 group-hover:text-primary transition-colors leading-tight">
              {title}
            </h3>
