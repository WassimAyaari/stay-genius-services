...
const Messages = () => {
  ...
  if (selectedContact) {
    return <div className="fixed inset-0 bg-background flex flex-col h-screen w-screen">
        ...
        <ScrollArea className="flex-1 px-2">
          <div className="space-y-3 py-4">
            {selectedContact.messages.map(message => <div key={message.id} className={cn("flex", message.sender === 'user' ? "justify-end" : "justify-start")}>
                <div className={cn("max-w-[80%] px-4 py-2 rounded-2xl mb-1", message.sender === 'user' ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-muted rounded-tl-none")}>
                  <p className="text-sm">{message.text}</p>
                  <div className={cn("flex items-center justify-end gap-1 mt-1", message.sender === 'user' ? "text-primary-foreground/80" : "text-muted-foreground")}>
                    <span className="text-[10px]">
                      {message.time}
                    </span>
                    {message.sender === 'user' && message.status && <span className="text-[10px] ml-1">
                        {message.status === 'read' && '✓✓'}
        ...
        <div className="border-t bg-card p-3 flex-shrink-0">
          ...
          }} placeholder="Type a message" className="resize-none min-h-0 h-10 py-2 px-4 rounded-full border-0 focus-visible:ring-1 bg-muted/50" />
            <Button size="icon" onClick={handleSendMessage} className="rounded-full h-10 w-10 flex-shrink-0" disabled={!inputMessage.trim()}>
  ...
  return <div className="fixed inset-0 bg-background flex flex-col h-screen w-screen">
      ...
      <div className="px-4 py-3 border-b bg-card">
        <input type="text" placeholder="Search messages..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full px-4 py-2 rounded-full bg-muted/50 text-sm focus:outline-none focus:ring-1 focus:ring-primary" />
      </div>
      ...
      <ScrollArea className="flex-1">
        <div className="p-2">
          {filteredContacts.length === 0 ? <div className="text-center py-8">
              <p className="text-muted-foreground">No contacts found</p>
            </div> : filteredContacts.map(contact => <div key={contact.id} className="cursor-pointer hover:bg-muted/50 transition-colors rounded-lg mb-1" onClick={() => setSelectedContact(contact)}>
                <div className="flex items-center gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {contact.lastMessage && <p className="text-sm truncate mt-1 my-[3px] py-0 px-0 text-left">{contact.lastMessage}</p>}
                  </div>
                </div>
              </div>)}
        </div>
