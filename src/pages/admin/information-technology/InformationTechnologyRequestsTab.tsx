...
const InformationTechnologyRequestsTab = ({
  ...
  // Find the IT category (accepts "Information technology", lowercases for robustness)
  const itCategory = categories.find(
    (cat) => cat.name?.toLowerCase().includes("information technology") || cat.name?.toLowerCase().includes("it")
  );

  // Filter IT requests
  const itRequests = requests
    .filter((req) => {
      const isIT =
        req.category_id === itCategory?.id ||
        req.type?.toLowerCase() === "information technology" ||
        req.type?.toLowerCase() === "it" ||
    ...
    <Card className="mb-6">
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>Information Technology Requests</CardTitle>
      ...
      <CardContent>
        ...
        {isRequestsLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            No Information Technology requests found.
          </div>
        )}
      </CardContent>
    </Card>
